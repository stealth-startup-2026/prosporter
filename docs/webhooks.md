...
2. The route polls the Storefront API uncached every 1.5 s until `isCaughtUp`
   reports an `updatedAt` at or after the webhook's `updated_at`, giving up
   after 20 s and expiring anyway (late-but-correct beats never). The API is
   eventually consistent across replicas (one read fresh, the next stale), so
   the first purge can still regenerate a stale page; the second pass below is
   what guarantees the outcome. Requiring two consecutive fresh probes was
   tried and only made edits slower (median 14.6 s vs 7.7 s). The probe sends the
